...
<br><br><br>
	<table class="table" border="1">
			...
			<tr>
				<td align="center" colspan="4">
					<img src="images/cobap_rh.png" width="120" style="float: left;margin-right: -100px;">
					 <div class="page-header" style="border: 0 !important;margin-bottom:-20px !important;margin-top: 0px;">
				      <h3>RELATÓRIO RESUMO NÍVEL OPERACIONAL</h3>
				    </div>
				</td>
			</tr>
		</thead>
		...
		<tr>
			...
			<td><b>AVALIADOR(ES):</b> <?php echo $this->dados['gestor'];?></td>
		</tr>
		...
		<tr><td colspan="3"><b>COMPORTAMENTAL</b></td></tr>
		<?php
		$total = 0;
		...
		for( $j=0; $j < count($valores); $j++ ){
			$script = "SELECT *,AVG(nota)AS media FROM resposta AS resp INNER JOIN pergunta AS pg ON resp.idpergunta = pg.idpergunta WHERE resp.idpergunta='".$valores[$j]."' AND avaliador IN(".$this->gst.") AND avaliado='".$this->id."'";
			...
			if( $temp ){
				...
				echo '<tr>';
					...
					echo '<td colspan="2">'.$datas['media'].'</td>';
				echo '</tr>';
		...
		<tr><td colspan="3"><p><b>Técnico: </b><?php echo ucfirst($this->dados['comentario_t']);?></p></td></tr>
	</table>
</div>
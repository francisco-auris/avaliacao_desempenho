...
class MAuto extends CConexao {
	...
	protected function searchFunc(){
		...
		if( $query ){
			while( $ftch = $query->fetch(PDO::FETCH_ASSOC) ){
				...
				$this->dados['level'] = $ftch['nivelavaliador'];
			...
			include_once "View/modal/hd_auto.php";
	...
	protected function geraAvaliacao(){
		...
		if( $serchPCT AND $serchPCT->rowCount() > 0 ){
			...
			echo '<form method="post" action="?window=auto&id='.$_GET['id'].'&action=salva&tag='.$this->pacote['titulo'].'">';
	...
	private function geraCabecalho(){
		...
		echo'<thead>';
		echo '<tr>';
			echo '<td class="gray" align="center" colspan="4"><h3>AVALIAÇÃO DE DESEMPENHO COMPETÊNCIAS COMPORTAMENTAIS</h3></td>';
		echo '</tr>';
		echo '</thead>';
		...
		echo '<tr style="font-size:12px;" class="grayb">';
			echo '<td align="center">ATENDE TOTALMENTE</td>';
			echo '<td align="center">ATENDE PARCIALMENTE</td>';
			echo '<td align="center">ATENDE RARAMENTE</td>';
			echo '<td align="center">NUNCA ATENDE</td>';
		echo '</tr>';
		echo '</table>';
	}
    ...
    protected function insertcomentario(){
    	$cconexao = $this->conectar();
    	$query = $cconexao->prepare("INSERT INTO coments VALUES (0, :avaliador, :avaliado, :comentario, 'A')");

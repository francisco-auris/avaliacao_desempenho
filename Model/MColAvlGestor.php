<?php
class MColAvlGestor extends CConexao {
	...
	protected function listColaboradores(){
		...
		if( isset($_GET['emp']) ){
			...
			$setor = $cconexao->query("SELECT * FROM funcionario WHERE empresa LIKE '".$emp."%' GROUP BY setor ORDER BY setor ASC");
			$setores = $cconexao->query("SELECT * FROM funcionario WHERE empresa LIKE '".$emp."%' GROUP BY setor ORDER BY setor ASC");
		}
		else {
			$setor = $cconexao->query("SELECT * FROM funcionario GROUP BY setor ORDER BY setor ASC");
			$setores = $cconexao->query("SELECT * FROM funcionario GROUP BY setor ORDER BY setor ASC");
		}
		

		if( $setor ){
			include_once "View/list/col_avl_gestor.php";
		}
		else {
	...
	protected function geraRelatorio(){
		include_once "Report/col_avl_gestor.php";
	}
}
?>